...
def pack_ota_app_bin(pack_dir: Path, output_bin: Path):
    ...
    flash_aes_enable = apps_part_info.get("aes_enable", False)
    # if flash_aes_enable is true, change the firmware name to origin_app.bin
    if flash_aes_enable:
        for part in sections:
            if part.get("firmware") == "app.bin":
                part["firmware"] = "origin_app.bin"
                break
...
def ota_pack():
    ...
    curr_project.build_summary += f"ota binary: {ota_bin.absolute()}\n"
