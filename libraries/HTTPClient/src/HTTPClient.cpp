...
static bool parseHttpTime(const char *str, struct tm *tm) {
#if defined(BK_IDK_HAVE_STRPTIME)
    return strptime(str, HTTP_TIME_PATTERN, tm) != nullptr;
#else
    (void)str;
...
#ifndef HTTPCLIENT_HAS_TLS
    if(protocol == "https") {
        log_e("HTTPS requires HTTPCLIENT_HAS_TLS");
        return false;
    }
#endif
...
bool HTTPClient::connect(void)
{
    ...
    // set Timeout for WiFiClient (milliseconds on BK-IDK)
    _client->setTimeout(_tcpTimeout);

    log_d(" connected to %s:%u", _host.c_str(), _port);

    // TLS connect just succeeded; do not re-probe via connected()/available() here
    // (WiFiClientSecure::available() may stop the session on a zero-length SSL poll).
    return true;
}
...
void HTTPClient::setCookie(String date, String headerValue)
{
    ...
    struct tm tm;
    if (parseHttpTime(date.c_str(), &tm)) {
    ...
    if (headerValue.indexOf("expires=") >= 0){
        ...
        if (parseHttpTime(value.c_str(), &tm)) {
