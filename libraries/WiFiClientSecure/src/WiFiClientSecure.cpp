...
{
    return sslclient->socket == rhs.sslclient->socket && _connected == rhs._connected;
}
...
int WiFiClientSecure::available()
{
    ...
    // 1) 已解密并缓存在 mbedTLS 内部缓冲区的字节
    int decoded = mbedtls_ssl_get_bytes_avail(&sslclient->ssl_ctx);
    if (decoded > 0) {
        return decoded + peeked;
    }

    // 2) 底层 socket 上是否已有可读数据；用零超时 select 探测，绝不阻塞。
    //    HTTPClient::connected() 会在发请求头之前调用 available()，
    //    如果这里对 SSL 层做阻塞读，就会导致 sendHeader() 被误判为失败
    //    （参考 BK7238 WiFiClientSecure：available() 只查询状态，不主动读）。
    if (sslclient->socket >= 0) {
        fd_set rfds;
        FD_ZERO(&rfds);
        FD_SET(sslclient->socket, &rfds);
        struct timeval tv = {0, 0};
        int sel = select(sslclient->socket + 1, &rfds, NULL, NULL, &tv);
        if (sel > 0 && FD_ISSET(sslclient->socket, &rfds)) {
            // 底层真的有数据，交给 mbedTLS 解一条记录。data_to_read 内部
            // 用的是 mbedtls_ssl_read(ssl_ctx, NULL, 0) + get_bytes_avail。
...
int WiFiClientSecure::setTimeout(uint32_t ms)
{
    // arduino_idk WiFiClient::setTimeout uses milliseconds
    _timeout = static_cast<int>(ms);
    _timeout_ms = ms;
    sslclient->socket_timeout = ms;
    mbedtls_ssl_conf_read_timeout(&sslclient->ssl_conf, ms);
    if (sslclient->socket >= 0) {
        struct timeval tv;
        tv.tv_sec = ms / 1000;
...
int WiFiClientSecure::setSocketOption(int option, char* value, size_t len)
{
    ...
    return lwip_setsockopt(sslclient->socket, SOL_SOCKET, option, value, len);
}
