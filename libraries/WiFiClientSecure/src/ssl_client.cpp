#include "Arduino.h"
#include <bk_idk_log.h>
#include <lwip/err.h>
#include <lwip/sockets.h>
#include <lwip/sys.h>
#include <lwip/netdb.h>

// mbedTLS 3.x hides struct members behind MBEDTLS_PRIVATE(); required for
// stop_ssl_socket / certificate fingerprint helpers in this translation unit.
...
// Match prebuilt libpsa_mbedtls before any other mbedTLS headers in this TU.
#include "mbedtls_psa_crypto_config.h"
...
#  warning "PSK cipher suites are disabled in the current mbedTLS build; setPreSharedKey() will not work."
#endif

const char *pers = "bk7239-tls";

#if defined(MBEDTLS_THREADING_C)
// Patch lock/unlock only; keep bk_threading init/free from libpsa_mbedtls.
// set_alt re-inits global mutexes (e.g. gmtime) and can leave invalid handles on BK7239N.
static bool arduino_mbedtls_handle_ok(const mbedtls_threading_mutex_t *mutex)
{
    if (mutex == NULL || !mutex->is_valid) {
        return false;
    }
    const uintptr_t handle = (uintptr_t)mutex->id;
    return handle >= 0x28040000U && handle <= 0x28080000U;
}

static void arduino_mbedtls_mutex_ensure_init(mbedtls_threading_mutex_t *mutex)
{
    if (arduino_mbedtls_handle_ok(mutex)) {
        return;
    }
    mutex->is_valid = 0;
    mutex->id = NULL;
...
static int arduino_mbedtls_mutex_lock(mbedtls_threading_mutex_t *mutex)
{
    if (mutex == NULL) {
        return MBEDTLS_ERR_THREADING_BAD_INPUT_DATA;
    }
    arduino_mbedtls_mutex_ensure_init(mutex);
    if (!arduino_mbedtls_handle_ok(mutex)) {
        return MBEDTLS_ERR_THREADING_BAD_INPUT_DATA;
    }
...
static int arduino_mbedtls_mutex_unlock(mbedtls_threading_mutex_t *mutex)
{
    if (mutex == NULL) {
        return MBEDTLS_ERR_THREADING_BAD_INPUT_DATA;
    }
...
static void arduino_mbedtls_threading_init_once(void)
{
    static bool done = false;
    if (done) {
        return;
    }
    mbedtls_mutex_lock = arduino_mbedtls_mutex_lock;
    mbedtls_mutex_unlock = arduino_mbedtls_mutex_unlock;
    done = true;
}
#endif
...
{
#if defined(MBEDTLS_THREADING_C)
    arduino_mbedtls_threading_init_once();
#endif
    ...
    // mbedtls_debug is not linked in the current bk_idk psa_mbedtls build
    (void)my_debug;
}
...
int start_ssl_client(sslclient_context *ssl_client, const IPAddress& ip, uint32_t port, const char* hostname, int timeout, const char *rootCABuff, bool useRootCABundle, const char *cli_cert, const char *cli_key, const char *pskIdent, const char *psKey, bool insecure, const char **alpn_protos)
{
    ...
    if (rootCABuff == NULL && pskIdent == NULL && psKey == NULL && !insecure && !useRootCABundle) {
        log_e("start_ssl_client: no CA/PSK and not insecure");
        return -1;
    }
    ...
    // BK7238: non-blocking socket + select() for connect completion.
    ...
    if (res < 0) {
        ...
        log_e("select returned due to timeout %d ms for fd %d", timeout, ssl_client->socket);
        lwip_close(ssl_client->socket);
        ssl_client->socket = -1;
        return -1;
    } else {
...
#define ROE(x,msg) { if (((x)<0)) { log_e("LWIP Socket config of " msg " failed."); return -1; }}
    ...
    // BK platform tls_connect.c uses MBEDTLS_SSL_MAX_FRAG_LEN_NONE (default).
    // Do not set max_frag_len here: 4096 MFL causes TLS handshake fragmentation
    // which mbedTLS rejects with MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE (-28800).

    mbedtls_ssl_conf_read_timeout(&ssl_client->ssl_conf, (uint32_t)ssl_client->socket_timeout);
    ...
    // MBEDTLS_SSL_VERIFY_REQUIRED if a CA certificate is defined
    // MBEDTLS_SSL_VERIFY_NONE if insecure or no CA is provided.
    if (insecure) {
...
#else
        log_e("PSK cipher suites are not enabled in mbedTLS");
        return -1;
#endif
    } else {
        return -1;
    ...
    while ((ret = mbedtls_ssl_handshake(&ssl_client->ssl_ctx)) != 0) {
        if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
            log_e("SSL/TLS handshake err...");
            return handle_error(ret);
        }
        if((millis()-handshake_start_time)>ssl_client->handshake_timeout)
        {
            log_e("mbedtls_ssl_handshake timeout...");
            return -1;
        }
        vTaskDelay(2);
    }
